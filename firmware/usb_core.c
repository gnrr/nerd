/*
 *
 * $Id: usb_core.c 41 2006-06-05 14:10:28Z gnrr $
 *
...
void isr_usb(void) interrupt 8 using 1;
void isr_resume(void) interrupt 6 using 2;
void init_interrupt_usb(void);


/* in nerd.c */
extern data uchar SendCurr[];
extern void core_apply_reset_values(void) using 1;
...
static void get_descriptor(void) using 1
{
    switch(sdat->wValueH) {
        ...
        case USB_DT_DEVICE:
            ...
            SUDPTRH = (uchar)((unsigned int)dev_desc >> 8);
        ...
        case USB_DT_CONFIG:
            EP0CS   = 0x02;                         /* clear HSNACK */
            SUDPTRH = (uchar)((unsigned int)conf_desc >> 8);
        ...
        case USB_DT_STRING:
            if(sdat->wValueL < NUM_STRING) {
                EP0CS   = 0x02;                     /* clear HSNACK */
                SUDPTRH = (uchar)(string_index[sdat->wValueL] >> 8);
        ...
        case USB_DT_HID:
            EP0CS   = 0x02;                         /* clear HSNACK */
            SUDPTRH = (uchar)(((unsigned int)conf_desc) + 18 >> 8);
        ...
        case USB_DT_REPORT:
            if(sdat->wIndexL == 0) {
                EP0CS   = 0x02;                     /* clear HSNACK */
                SUDPTRH = (uchar)((unsigned int)(report_desc_keyboard) >> 8);
...
void isr_usb(void) interrupt 8 using 1
{
    ...
    if(IN07IRQ & 0x01) {
        IN07IRQ = 0x01;         /* clear IN0 interrupt */
    }

    /* was this a OUT0 interrupt? */
    if(OUT07IRQ & 0x01) {
        OUT07IRQ = 0x01;        /* clear OUT0 interrupt */

        /* is this packet the keyboard DATA stage of a control transfer? */
        if(is_ctrl_out0_kbd) {
#if ENABLE_LED
            uchar led_status = out0buf(0);
#endif
            /*
        ...
        OUT0BC = 0x00;          /* arm endpoint again */
    }


    /* was this the URES interrupt? */
    if(USBIRQ & 0x10) {
        USBIRQ = 0x10;          /* clear request */
    ...
    if(USBIRQ & 0x08) {
        USBIRQ = 0x08;          /* clear request */
        PCON |= 0x01;           /* enter idle mode */
    }

    /* was this an IN1 interrupt? */
    if(IN07IRQ & 0x02) {
        IN07IRQ = 0x02;         /* clear IN1 interrupt */
        in1_busy = FALSE;       /* clear busy flag */
    }
...
void isr_resume(void) interrupt 6 using 2
{
    EICON &= ~0x10;             /* clear resume interrupt */
}
...
void init_interrupt_usb(void)
{
    USBBAV   = 0x00;            /* clear autovector enable */
#if EZ_DOWNLOAD
    USBIRQ   = 0xff; /* clear SUDAV interrupt and all other interrupts */
#endif

    USBIEN   = 0x19;      /* enable SUDAV, SUSP and URES interrupts */
#if EZ_DOWNLOAD
    IN07IRQ  = 0x03;            /* clear IN0 and IN1 interrupts */
#endif
    IN07IEN  = 0x03;            /* enable IN0 and IN1 interrupts */
#if EZ_DOWNLOAD
    OUT07IRQ = 0x01;            /* clear OUT0 interrupt */
#endif
    OUT07IEN = 0x01;            /* enable OUT0 interrupt */

#if EZ_DOWNLOAD
    IN1CS    = 0x00;            /* unstall endpoints */
#endif
    ...
    EUSB     = 1;               /* enable external 2 interrupt */

    EICON   &= ~0x10;           /* clear pending resume interrupt */
    EICON   |= 0x20;            /* enable resume interrupt */
}
